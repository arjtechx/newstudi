...
import { getQuestions, saveQuestion, deleteQuestion, getSubjects } from '@/lib/store';
import { Question, Difficulty, Subject } from '@/lib/types';
...
import { generateQuestion } from '@/ai/flows/automated-question-generation-flow';
  ...
  const handleRunAiGeneration = async () => {
    ...
    try {
      const result = await generateQuestion({
        ...
        difficulty: aiConfig.difficulty === 'Hardcore' ? 'Difícil' : aiConfig.difficulty as any,
      ...
      setEditingQuestion({
        ...
        nivelDificuldade: aiConfig.difficulty,
        explicacao: result.explicacao,
        banca: 'INTELIGÊNCIA ARTIFICIAL',
        ano: new Date().getFullYear(),
        tags: [result.materia, result.assunto]
      });

      setIsAiDialogOpen(false);
      setIsQuestionDialogOpen(true);
      toast({ title: 'Questão Gerada!', description: 'Revise os dados antes de salvar.' });
    } catch (error: any) {
      toast({ variant: 'destructive', title: 'Erro na IA', description: 'Não foi possível gerar a questão no momento.' });
    } finally {
        ...
        <DialogContent className="max-w-md border-4 border-primary/20 rounded-[2.5rem]">
          <DialogHeader>
            ...
            <DialogDescription className="font-bold">
              O motor Gemini 2.0 criará uma questão inédita baseada no tema.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6 py-4">
             <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-primary tracking-widest">Setor (Matéria)</label>
                <Select value={aiConfig.subject} onValueChange={v => setAiAiConfig({...aiConfig, subject: v})}>
             ...
             <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-primary tracking-widest">Alvo Específico (Tópico)</label>
                <Input 
                  placeholder="Ex: Artigo 5º da CF ou Crase" 
                  className="h-12 border-2 font-bold"
                  value={aiConfig.topic}
                  onChange={e => setAiAiConfig({...aiConfig, topic: e.target.value})}
                />
             </div>
             <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-primary tracking-widest">Nível de Dificuldade IA</label>
                <div className="grid grid-cols-3 gap-2">
                   {['Fácil', 'Médio', 'Difícil'].map(d => (
                     <Button 
                        key={d} 
                        variant={aiConfig.difficulty === d ? 'default' : 'outline'} 
                        onClick={() => setAiAiConfig({...aiConfig, difficulty: d as any})}
                        className="font-black text-[10px] h-10"
                     >
                       {d}
                     </Button>
                   ))}
          ...
          <DialogFooter>
             <Button 
                className="w-full h-16 text-lg font-black gap-3 shadow-[0_6px_0_0_#1e40af] active:translate-y-1 active:shadow-none"
