...
export const LessonSchema = z.object({
  ...
  title: z.string().trim().min(1, "O título da lição é obrigatório."),
...
export const ModuleSchema = z.object({
  id: z.string().optional(),
  title: z.string().trim().min(1, "O título do módulo é obrigatório."),
...
export const QuestionSchema = z.object({
    ...
    assunto: z.string({ required_error: "O campo 'assunto' é obrigatório." }).trim().min(1, "O campo 'assunto' não pode estar vazio."),
    enunciado: z.string({ required_error: "O campo 'enunciado' é obrigatório." }).trim().min(1, "O campo 'enunciado' não pode estar vazio."),
    alternativas: z.array(z.string()).min(2, "A questão deve ter pelo menos 2 alternativas."),
    correta: z.coerce.number({ invalid_type_error: "O campo 'correta' deve ser um número." }).int(),
    explicacao: z.string({ required_error: "O campo 'explicacao' é obrigatória." }).trim().min(1, "O campo 'explicacao' não pode estar vazio."),
    ...
    tags: z.array(z.string()).optional().default([]), // tags centrais
