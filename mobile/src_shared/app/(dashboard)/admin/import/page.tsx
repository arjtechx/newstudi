...
export default function AdvancedImportPage() {
  ...
  const handleAnalyze = async () => {
    ...
    if (!firestore) {
      toast({ variant: "destructive", title: "Erro", description: "Firebase não inicializado. Tente recarregar a página." });
      return;
    }
  ...
  const handleImportSelected = async () => {
    ...
    try {
      ...
      // Se for erro de validação Zod, mostrar campos específicos
      if (err?.errors) {
        const details = err.errors.map((e: any) => `${e.path.join('.')}: ${e.message}`).slice(0, 3).join('\n');
        toast({ variant: "destructive", title: "Erro de Validação", description: details });
      } else {
        toast({ variant: "destructive", title: "Erro na Gravação", description: err.message || "Verifique o console para detalhes." });
      }
    } finally {
      setIsProcessing(false);
    }
          ...
          <Card className="material-card border-2 overflow-hidden">
            ...
            <CardHeader>
              <CardTitle className="text-base font-black uppercase italic flex items-center gap-2">
                <FileJson className="w-4 h-4 text-amber-500" /> Guia de Formato JSON
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">📚 Questão (campos obrigatórios)</p>
                <pre className="bg-slate-950 text-emerald-400 text-[9px] font-mono p-3 rounded-xl overflow-x-auto leading-relaxed">{`{
  ...
  "explicacao": "Porque..."
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">📦 Múltiplas questões (array)</p>
                <pre className="bg-slate-950 text-blue-400 text-[9px] font-mono p-3 rounded-xl overflow-x-auto leading-relaxed">{`[
  { "materia": "...", "enunciado": "...", ... },
  { "materia": "...", "enunciado": "...", ... }
]`}</pre>
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">🗂️ Backup completo</p>
                <pre className="bg-slate-950 text-purple-400 text-[9px] font-mono p-3 rounded-xl overflow-x-auto leading-relaxed">{`{
  "questions": [ {...}, {...} ],
  "courses":   [ {...}, {...} ]
}`}</pre>
              </div>
