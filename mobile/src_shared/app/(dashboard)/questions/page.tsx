...
type DifficultyMode = 'AUTO' | 'MANUAL' | 'SURVIVAL';
...
export default function QuestionsPage() {
  ...
  const startSession = async () => {
    ...
    } else if (difficultyMode === 'SURVIVAL') {
      // Modo sobrevivência pega qualquer dificuldade e mistura, mas vamos tentar ordenar da mais fácil para a mais difícil
      const order = { 'Fácil': 1, 'Médio': 2, 'Difícil': 3, 'Hardcore': 4 };
      subset = subset.sort((a, b) => (order[a.nivelDificuldade as keyof typeof order] || 2) - (order[b.nivelDificuldade as keyof typeof order] || 2));
      // Vamos pegar até 100 questões (basicamente "infinito" prático para uma sessão)
    } else {
      if (!user) {
        toast({
          variant: "destructive",
    ...
    if (difficultyMode === 'SURVIVAL') {
      subset = subset.slice(0, 100); // Até 100 questões
      setHp(3);
      setCombo(0);
    } else {
      // Mistura apenas nos outros modos, sobrevivência tem progressão
    ...
    if (difficultyMode === 'SURVIVAL') {
      toast({
        ...
        description: "Você tem 3 vidas. Cada erro custa caro. Quanto tempo você consegue resistir?",
      });
  ...
  const handleGameOver = () => {
    audioManager.playError(); // Ou um som de game over específico se existir
    setView('RESULT');
    toast({
      variant: "destructive",
      title: "GAME OVER",
      description: `Você sobreviveu por ${sessionResults.correct} rodadas.`,
    });
  };
  ...
  const submitResult = (ans: number | null, isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      // Bônus de Combo no modo sobrevivência
      if (difficultyMode === 'SURVIVAL') {
        const comboMultiplier = 1 + (combo * 0.1); // +10% por acerto consecutivo
      ...
      if (difficultyMode === 'SURVIVAL') {
        ...
        setHp(prev => {
          const newHp = prev - 1;
          if (newHp <= 0) setTimeout(handleGameOver, 2000); // Game over após ver a resposta
          return newHp;
        });
      }
    }
  ...
  const handleNext = () => {
    if (difficultyMode === 'SURVIVAL' && hp <= 0) {
      handleGameOver();
      return;
    }
          ...
          <CardContent className="space-y-8">
            <div className="space-y-4">
              ...
              <Tabs defaultValue="AUTO" onValueChange={(v) => setDifficultyMode(v as DifficultyMode)} className="w-full">
                <TabsList className="grid grid-cols-3 w-full h-12 bg-muted/30">
                  <TabsTrigger value="AUTO" className="gap-2 font-bold uppercase text-xs">
                    <Cpu className="w-4 h-4" /> Adaptativo
                  </TabsTrigger>
                  <TabsTrigger value="MANUAL" className="gap-2 font-bold uppercase text-xs">
                    <User className="w-4 h-4" /> Manual
                  </TabsTrigger>
                  <TabsTrigger value="SURVIVAL" className="gap-2 font-bold uppercase text-xs text-red-500 data-[state=active]:text-red-600 data-[state=active]:border-red-500">
                ...
                <TabsContent value="SURVIVAL" className="pt-4">
                  <p className="text-xs text-red-600 font-bold italic bg-red-50 p-3 rounded-lg border border-red-200">
                    MODO DE ELIMINAÇÃO: Você começa com 3 Vidas (Corações). Cada erro tira 1 vida. O jogo continua até você perder todas. Acerte em sequência para ganhar Bônus de Combo de XP!
                  </p>
                </TabsContent>
              </Tabs>
            </div>
            ...
            <div className={cn("space-y-3 transition-all", difficultyMode === 'SURVIVAL' && "opacity-50 pointer-events-none")}>
                <Label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest flex items-center gap-2">
                    <Layers className="w-3 h-3" /> Volume de Alvos (Nº de Questões)
                </Label>
                <Input
                    type="number"
                    value={difficultyMode === 'SURVIVAL' ? 999 : numQuestions}
                    disabled={difficultyMode === 'SURVIVAL'}
                    onChange={(e) => {
      ...
      <div className="max-w-2xl mx-auto space-y-8 animate-in zoom-in-95 duration-700">
        <div className="text-center space-y-4">
          <div className="relative inline-block">
             <Trophy className={cn("w-24 h-24 mx-auto", hp <= 0 ? "text-slate-400 drop-shadow-none" : "text-yellow-500 drop-shadow-[0_0_15px_rgba(234,179,8,0.5)]")} />
             {hp > 0 && (
          ...
          <h1 className="text-5xl font-black uppercase italic tracking-tighter">
            {hp <= 0 ? 'GAME OVER' : 'Missão Cumprida'}
          </h1>
          <p className="text-muted-foreground font-bold tracking-[0.2em] text-xs uppercase">
            {hp <= 0 ? 'Você foi abatido em combate' : 'Debriefing de Operação'}
          </p>
        </div>
  ...
  return (
    <div className={cn(
      "max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 items-start transition-all",
      isHardcore && "animate-stress"
    )}>
      {/* Coluna Principal da Questão */}
      <div className="flex-1 w-full space-y-6">
      
      {/* Barra de Status (HP e Combo no Survival) */}
      {difficultyMode === 'SURVIVAL' && (
        <div className="flex items-center justify-between bg-card border-2 border-red-500/20 rounded-2xl p-4 shadow-lg animate-in slide-in-from-top-4">
          <div className="flex items-center gap-3">
            <span className="text-xs font-black uppercase tracking-widest text-muted-foreground">Vidas:</span>
            <div className="flex gap-1">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="transition-all duration-300">
                  {i < hp ? (
                    <Heart className="w-8 h-8 fill-red-500 text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.6)] animate-pulse" />
                  ) : (
          ...
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-black uppercase tracking-widest text-orange-500">Combo Atual</span>
            <div className="flex items-center gap-1 text-2xl font-black text-orange-600">
              {combo}x <Flame className="w-5 h-5 fill-current animate-bounce" />
            </div>
          </div>
        </div>
      )}
      ...
      <div className={cn(
        ...
        <div className="flex flex-col items-center">
          <div className="flex gap-1 mb-1">
            {difficultyMode !== 'SURVIVAL' && filteredQuestions.map((_, i) => (
              <div 
                key={i} 
                className={cn(
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">
            {difficultyMode === 'SURVIVAL' ? `Rodada: ${currentIdx + 1}` : `Progresso: ${currentIdx + 1}/${filteredQuestions.length}`}
          </span>
        </div>
