...
export default function CoursesPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex overflow-x-auto snap-x snap-mandatory md:grid md:grid-cols-2 lg:grid-cols-3 gap-6 pb-6 -mx-4 px-4 md:mx-0 md:px-0 no-scrollbar">
        {filteredCourses.map((course) => {
          const progress = getCourseProgress(course, user?.completedLessons || []);
          return (
            <Card key={course.id} className="material-card overflow-hidden flex flex-col group border-2 hover:border-primary/30 min-w-[85vw] snap-center shrink-0 md:min-w-0 md:shrink">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-6 text-xs font-bold text-muted-foreground uppercase pt-2">
                  <div className="flex items-center gap-2">
                    ...
                    {course.modules.length} Matérias
                  </div>
                  <div className="flex items-center gap-2">
