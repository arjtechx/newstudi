...
>(({ className, variant, ...props }, ref) => {
  const handleCopy = (e: React.MouseEvent) => {
    // Evita copiar se clicar no botão de fechar ou em ações
  ...
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(toastVariants({ variant }), className, "cursor-pointer active:scale-95 transition-transform")}
      onClick={handleCopy}
      title="Clique para copiar mensagem"
      {...props}
    />
  )
})
